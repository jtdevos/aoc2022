...
def test1():
    ...
    for x in range(-10, 10):
        for y in range(-10, 10):
            p = (x,y)
            debug(f'orig:{p}\t clamp:{clamp_tuple(p)}')
...
def test2(): 
    ...
    ph.move(1,1)
    ph.move(-1,1)
    debug(f'Point ph: {ph.pos()}')
    debug(f'Point ph history: {ph.pos_history}')
...
def run_commands(commands, chainsize):
    ...
    for code, rpt in commands:
         ...
         debug(f"Command:{code}->{v}\t times:{rpt}")
         ...
         for _ in range(rpt):
            ...
            for i in range(1, chainsize):
                tph = pchain[i-1]
                tpt = pchain[i]
                debug(f"  segment[{i-1}]:{tph}\t segment[{i}]:{tpt}\t dist:{tph.diff(tpt).dist()}")
                if(tph.diff(tpt).dist()) >= 2:
                    debug(f"    Distance too great -- scooting tail")
                    scootVec = tph.diff(tpt).clamp()
                    tpt.move(scootVec.x, scootVec.y)
                    debug(f"    New tail position:{tpt} (distance:{tph.diff(tpt).dist()})")
    debug("*"*20)
    debug(f"Tail history:\t {pt.pos_history}")
    tc = set(pt.pos_history)

    debug(f"Tail coverage:\t {tc}")
